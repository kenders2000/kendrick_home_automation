...
class TOF_Sense():
    ...
    def __init__(self, bus = 0,TOF_SCL = 15,TOF_SDA = 16):
        self.bus = SoftI2C(scl = Pin(TOF_SCL) ,sda = Pin(TOF_SDA),freq = 400000)
    
    def read(self,reg):
        data = self.bus.readfrom_mem(TOF_addr, reg, 1)
        return data[0]

    def read_word(self,reg):
        data = self.bus.readfrom_mem(TOF_addr, reg, 2)
        return ((data[1] * 256 ) + data[0])

    def write(self,reg,data):
        temp = [0]
        temp[0] = data & 0xFF
        self.bus.writeto_mem(TOF_addr,reg,bytearray(temp))

    def write_word(self,reg,data):
        temp = [0,0]
        temp[0] = data & 0xFF
        temp[1] =(data & 0xFF00) >> 8
        self.bus.writeto_mem(TOF_addr,reg,temp)

    def I2C_Read_Nbyte(self,reg,num):
        data = self.bus.readfrom_mem(TOF_addr, reg, num)
        return data
...
#             tof.IIC_Change_Mode_To_UART()  # Switch I2C mode to UART query mode I2C模式切换成UART查询模式
            sleep(0.02)#The refresh rate defaults to 50HZ. If the refresh rate is set to 100HZ, the time here is 1/100=0.01 刷新率默认为50HZ,如果刷新率设置成100HZ,则这里的时间为1/100=0.01s
