...
class TOF_Sense():
    ...
    def TOF_Active_Decoding(self):    
        if self.ser.any() > 0: #Waiting for serial port data 等待串口数据
            ...
            if self.count_i > 15:#If the number of received data is greater than 15, the count variable can be cleared and a decoding can be performed. 接收数量大于15,则可以将计数变量清零并进行一次解码
                ...
                if (TOF_rx_data[0] == TOF_FRAME_HEADER) and (TOF_rx_data[1] == TOF_FUNCTION_MARK) and (self.check_sum == TOF_rx_data[15]):
                    ...
                    print("")
                    while self.ser.read() != None:
                        sleep(0.001) #Clear the serial port input register 清空串口输入寄存器
                else:
    ...
    def TOF_Inquire_Decoding(self,id):
        ...
        sleep(0.015) #Waiting for the sensor to return data 等待传感器返回数据
...
if __name__ == "__main__":
    ...
    try:
        while True:
            tof.TOF_Active_Decoding()    # Active Output Example 主动输出示例
#             tof.TOF_Inquire_Decoding(0)  # Example query output 查询输出示例
